...
class LDE():#Lista Duplamente Encadeada
    ...
    def __str__(self):
        if self.isEmpty():
            return " "
        nodeAtual = self.getFirstNode()
        string = ""
        while nodeAtual is not None:
            if nodeAtual == self.lastNode:
                string+= str(nodeAtual.getData())
...
entrada = input().split()
for i in range(quantPessoas):
    fila.InsertAtEnd(entrada[i])
qntRmv = int(input())
entrada2 = input().split()
for j in range(qntRmv):
    fila.RemoveAtMiddle(entrada2[j])
print(fila)

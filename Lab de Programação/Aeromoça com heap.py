...
def Dijkstra(g,s):#aqui eu passo o grafo e um valor(int) do grafo inicial
    ...
    for i in g.vertexes:
        Q.push((i.distance,i.getVertex()))
    while Q.isEmpty() is not True:
        u = Q.pop()#pego o vertice com menor distancia
        for viz in g.vertexes[u[1]].vizinhos:
            if u[0] + g.edges[u[1]][viz.getVertex()] < viz.distance:#se a distancia do menor + o peso da aresta for < que a dist do vizinho
                viz.distance = u[0] + g.edges[u[1]][viz.getVertex()] # a distancia do vizinho vai ser a nova menor distancia
                Q = MinHeap([(x.distance,x.getVertex()) for x in g.vertexes])
                Q.heap.remove(u)
